...
vi.mock('./blocker', () => ({
  default: ({ index, blocker, onUpdate, onResolve, onDelete }: any) => (
    <div data-testid={`blocker-${index}`}>
      <span data-testid="blocker-description">{blocker.description}</span>
      <span data-testid="blocker-resolved">{blocker.isResolved.toString()}</span>
      <button 
...
describe('Blockers', () => {
  ...
  describe('Multiple interactions', () => {
    ...
    it('should handle resolve and delete operations on different blockers', async () => {
      ...
      const mockOnResolve = vi.fn();
      const mockOnDelete = vi.fn();
      render(<Blockers {...defaultProps} onResolve={mockOnResolve} onDelete={mockOnDelete} />);
  ...
  describe('Props propagation', () => {
    it('should pass all required props to Blocker components', () => {
      ...
      expect(screen.getByTestId('blocker-0')).toBeInTheDocument();
      expect(screen.getByTestId('blocker-description')).toHaveTextContent('First blocker');
    });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle blockers with empty descriptions', () => {
      ...
      render(<Blockers {...defaultProps} blockers={emptyDescriptionBlockers} />);
    ...
    it('should handle very long blocker lists', () => {
      const manyBlockers: Impediment[] = Array.from({ length: 100 }, (_, i) => ({
    ...
    it('should maintain correct indices with many blockers', async () => {
      ...
      render(<Blockers {...defaultProps} blockers={manyBlockers} onDelete={mockOnDelete} />);
  ...
  describe('Component lifecycle', () => {
    it('should update when blockers prop changes', () => {
      ...
      const newBlockers = [...mockBlockers, { description: 'Fourth blocker', isResolved: false }];
    ...
    it('should handle callback function changes', async () => {
      ...
      const { rerender } = render(<Blockers {...defaultProps} onAdd={firstOnAdd} />);
  ...
  describe('Accessibility', () => {
    it('should render components in logical order', () => {
      ...
      expect(blockerElements[0].compareDocumentPosition(addBlockerElement))
